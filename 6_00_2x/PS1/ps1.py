...
def greedy_cow_transport(cows, limit=10):
    ...
    while len(taken_cows) < len(cows):
        ...
        for cow, weight in cows_sorted:
            if weight <= available_weight and cow not in taken_cows:
                taken_cows.append(cow)
                this_trip.append(cow)
...
def brute_force_cow_transport(cows, limit=10):
    ...
    """
    # TODO: Your code here
    trip_options = get_partitions(cows.keys())
    best = None
    for test_trips in trip_options:
        ...
        if is_valid:
            ...
            if best is None:
                ...
                best = test_trips
            elif len(test_trips) < len(best):
                # Current valid trip set needs to have fewer trips than best
                best = test_trips
